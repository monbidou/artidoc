...
export default function ModifierFacturePage() {
  ...
  // ── Chargement initial : facture (champs principaux) ──
  useEffect(() => {
    if (!facture || loaded) return
    setDateFacture(facture.date_emission || '')
    setDateEcheance(facture.date_echeance || '')
    setObjet(facture.objet || '')
    setChantierId(facture.chantier_id || null)

    // Conditions de paiement (fallback sur notes legacy si vide)
    setConditions(facture.conditions_paiement || facture.notes || DEFAULT_CONDITIONS_PAIEMENT)
    setNotesPerso(facture.notes_personnalisees || '')

    // Forfait (si la colonne existe et > 0)
    if (facture.forfait_ht && facture.forfait_ht > 0) {
      setUseForfait(true)
      setForfaitHT(facture.forfait_ht)
    }

    // Acompte
    if ((facture.acompte_pourcent && facture.acompte_pourcent > 0) || (facture.acompte_montant_ttc && facture.acompte_montant_ttc > 0)) {
      setAcompteActive(true)
  ...
  // ── Chargement client (depuis client_id puis fallback notes_client) ──
  useEffect(() => {
    if (!facture || !loaded) return
    if (clientNom) return // déjà chargé
    ...
    if (facture.client_id) {
      supabase.from('clients').select('*').eq('id', facture.client_id).maybeSingle().then(({ data }) => {
        if (data) {
          ...
          if (facture.client_nom) setClientNom(facture.client_nom)
          if (facture.client_adresse) setClientAdresse(facture.client_adresse)
        }
      })
    } else {
      if (facture.client_nom) setClientNom(facture.client_nom)
      ...
      if (data && data.length > 0) {
        ...
        setLines(raw.map((l, i) => {
          ...
          return {
            ...
            qty: l.quantite || (reactType === 'line' ? 1 : 0),
            unit: l.unite || 'U',
        ...
        nextId += raw.length
        // Récupère le taux TVA depuis la première ligne de type "line"
        const firstLine = raw.find(l => !l.type || l.type === 'prestation')
        if (firstLine && firstLine.taux_tva != null) {
          setGlobalTvaRate(firstLine.taux_tva)
          setTvaUserOverride(true) // on respecte la TVA déjà choisie sur cette facture
        }
      }
  ...
  // ── Autocomplete chantier sur "Objet" ──
  ...
  const handleSave = useCallback(async (action: 'brouillon' | 'enregistrer') => {
    ...
    try {
      ...
      const factureData: Record<string, unknown> = {
        ...
        date_emission: dateFacture || null,
    ...
    <div className="min-h-screen">
      ...
      <div className="sticky top-0 bg-white border-b border-gray-200 z-10 py-3 px-6 flex items-center justify-between">
        <div className="flex items-center gap-3">
          <Link href={'/dashboard/factures/' + id} className="p-1.5 rounded-md hover:bg-gray-100">
            <ArrowLeft size={18} className="text-[#6b7280]" />
          </Link>
          <h2 className="hidden sm:block font-syne font-bold text-lg text-[#1a1a2e]">Modifier {facture.numero}</h2>
        </div>
      ...
      <div className="p-6 space-y-6">
        ...
        <div className="flex flex-wrap items-center gap-3 justify-end pb-8">
          <Link href={'/dashboard/factures/' + id}
            className="h-12 px-6 rounded-xl border-2 border-gray-300 text-sm font-syne font-semibold text-gray-600 hover:border-gray-400 hover:bg-gray-50 transition-all flex items-center">
            Annuler
          </Link>
          <button onClick={() => handleSave('brouillon')} disabled={saving}
